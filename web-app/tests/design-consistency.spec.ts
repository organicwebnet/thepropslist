...
test.describe('Design Consistency & Visual Regression Tests', () => {
        ...
        for (const element of textElements) {
          if (await element.isVisible()) {
            ...
            if (textColor.includes('0, 0, 0') && backgroundColor.includes('0, 0, 0')) {
              const elementText = await element.textContent() || 'Unknown element';
              const elementTag = await element.evaluate(el => el.tagName);
              const elementClass = await element.getAttribute('class') || 'no-class';
              const elementId = await element.getAttribute('id') || 'no-id';
              const elementStyle = await element.getAttribute('style') || 'no-style';
              throw new Error(`Black text on black background detected on ${pagePath}\nURL: ${page.url()}\nElement: ${elementTag} (class: ${elementClass}, id: ${elementId})\nStyle: ${elementStyle}\nText: "${elementText.substring(0, 100)}..."\nText Color: ${textColor}\nBackground Color: ${backgroundColor}`);
            }
          }
        }
      }
  ...
  test.describe('Layout Consistency', () => {
    test('should maintain consistent spacing and layout across pages', async ({ page }) => {
      // Only test authenticated pages that should have headers
      const pages = ['/props'];
      const layoutMetrics = [];
      ...
      for (const pagePath of pages) {
        ...
        const mainBox = await page.locator('main, .main, .content').first().boundingBox();
    ...
    test('should have consistent container widths', async ({ page }) => {
      ...
      for (const pagePath of testPages) {
        ...
        // Check that main containers use consistent max-width classes
        const containers = await page.locator('[class*="max-w-"]').all();
        
        // Should have consistent container sizing
    ...
    test('should maintain consistent padding and margins', async ({ page }) => {
      ...
      const cards = await page.locator('.card, .bg-white, .bg-pb-darker').all();
  ...
  test.describe('Component Consistency', () => {
    test('should have consistent button styles across pages', async ({ page }) => {
      const pages = ['/login', '/signup', '/props'];
      const buttonStyles = [];
      ...
      expect(uniqueBorderRadiuses.length).toBeLessThan(5); // Not too many variations
    });
    ...
    test('should have consistent card/container styles', async ({ page }) => {
      ...
      const cards = await page.locator('.card, .bg-white, .bg-pb-darker, .rounded').all();
      const cardStyles = [];
      ...
      if (cardStyles.length > 1) {
        ...
        const uniqueBorderRadiuses = [...new Set(borderRadiuses)];
        expect(uniqueBorderRadiuses.length).toBeLessThan(4); // Consistent border radius
      }
    });
  });
  ...
  test.describe('Typography Consistency', () => {
    test('should have consistent heading styles', async ({ page }) => {
      ...
      if (h1Styles.length > 1) {
        ...
        expect(uniqueH1Sizes.length).toBeLessThan(3); // H1 should be consistent
      }

      if (h2Styles.length > 1) {
        const h2FontSizes = h2Styles.map(s => s.fontSize);
        const uniqueH2Sizes = [...new Set(h2FontSizes)];
        expect(uniqueH2Sizes.length).toBeLessThan(3); // H2 should be consistent
      }
    });
    ...
    test('should have consistent text colors', async ({ page }) => {
      ...
      expect(uniqueColors.length).toBeLessThan(10); // Reasonable color palette
    });
  });
