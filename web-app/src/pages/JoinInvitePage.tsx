...
const JoinInvitePage: React.FC = () => {
  ...
  const completeAcceptance = async (currentUser: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await completeAcceptance(currentUser);
    } catch (err) {
  ...
  const handleGoogle = async () => {
    try {
      ...
      if (profile) {
        ...
        await completeAcceptance(profile);
      }
    } catch (e) {
      ...
      <div className="min-h-screen w-full bg-gradient-dark flex items-center justify-center px-4 py-10">
        {loading ? (
          ...
          <div className="w-full max-w-4xl bg-pb-darker/60 rounded-xl shadow-lg p-6">
            <h1 className="text-2xl font-bold text-white mb-2">Join show</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Google column */}
              <div className="rounded-lg border border-pb-primary/20 p-4 bg-pb-darker/30">
                ...
                <button type="button" onClick={handleGoogle} className="w-full justify-center px-3 py-2 rounded bg-white text-[#1a1a1a] font-semibold shadow hover:bg-gray-100 flex items-center">
              ...
              {/* Email/password column */}
              <form onSubmit={handleSubmit} className="space-y-3 rounded-lg border border-pb-primary/20 p-4 bg-pb-darker/30">
                <div>
