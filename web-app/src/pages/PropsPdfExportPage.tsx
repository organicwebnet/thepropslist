...
function generatePropsListPages(
  ...
  const titlePageHtml = options.includeTitlePage ? `
    <div class='page'>
      <div class='header' style='text-align:center;'>
        ${logoUrl ? `<img src='${logoUrl}' class='logo' style='max-height:72px' />` : ''}
      </div>
      <div style='flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:48px;'>
        <div style='font-size:3em;font-weight:900;margin-bottom:12px;'>${options.title || 'Props Bible'}</div>
        ${showData?.venue ? `<div style='font-size:1.2em;color:#666;'>${showData.venue}</div>` : ''}
      </div>
      <div class='footer' style='text-align:center;'>
        <span style='font-size:1em;color:#888;'>Generated by The Props List</span>
      </div>
    </div>
  ` : '';
  ...
  `;
  // Contacts page (optional)
  const contacts = (() => {
    const rows: string[] = [];
    // team map: { uid: role } or collaborators list
    if (showData?.team && typeof showData.team === 'object') {
      for (const [uid, role] of Object.entries(showData.team)) {
        rows.push(`<tr><td>${role}</td><td>${uid}</td><td></td><td></td></tr>`);
      }
    } else if (Array.isArray(showData?.collaborators)) {
      for (const c of showData.collaborators) {
        rows.push(`<tr><td>${c.role || ''}</td><td>${c.name || c.email || ''}</td><td>${c.email || ''}</td><td>${c.phone || ''}</td></tr>`);
      }
    }
    if (rows.length === 0) return '';
    return `
      <div class='page'>
        <div class='header'><div style='font-size:1.6em;font-weight:800;'>Contacts</div></div>
        <div class='toc-content'>
          <table class='toc-table'>
  ...
  const pages = [titlePageHtml, tocHtml, ...propPages, (options.includeContacts ? contacts : '')].filter(Boolean) as string[];
  const fullHtml = `
...
const PropsPdfExportPage: React.FC = () => {
  ...
  const handlePreview = async () => {
    ...
    const { pages, fullHtml, css } = generatePropsListPages(sortedProps, { ...pdfOptions, title: showTitle }, header, footer, logoUrl, showData);
  ...
  const pageH = pdfOptions.orientation === 'landscape' ? 794 : 1123;
