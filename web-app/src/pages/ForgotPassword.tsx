...
export default function ForgotPassword() {
  const { resetPassword, loading, error, clearError } = useWebAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email) {
      setDebugInfo('Email is required');
      return;
    }
    try {
      ...
      setDebugInfo(`Reset error: ${err.code || 'Unknown error'} - ${err.message || 'No message'}`);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-white/10 backdrop-blur-xl p-8 rounded-3xl shadow-2xl border border-white/20 relative overflow-hidden z-10">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            {loading ? <Loader2 className="animate-spin w-5 h-5" /> : 'Send Reset Email'}
          </button>
        </form>
