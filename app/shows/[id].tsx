...
export default function ShowDetailScreen() {
  ...
  const { shows, selectedShow, setSelectedShowById, loading: showsLoading } = useShows();
  ...
  useEffect(() => {
    if (id && !showToDisplay && !showsLoading) {
      ...
      if (foundShow) {
        setSelectedShowById(id); // This might trigger a re-render if selectedShow updates
      } else {
        // Handle show not found, perhaps navigate back or show error
        console.warn(`Show with ID ${id} not found in shows list.`);
      }
    }
  }, [id, shows, showToDisplay, showsLoading, setSelectedShowById]);
